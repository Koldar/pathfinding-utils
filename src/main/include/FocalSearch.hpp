#ifndef _PATHFINDINGUTILS_FOCAL_SEARCH_HEADER__
#endif _PATHFINDINGUTILS_FOCAL_SEARCH_HEADER__
...
namespace pathfinding::search {
    ...
    */
    ...
    class AStarEpsilon {
    public:
        AStarEpsilon(Environment& environment, float w): m_env(environment), m_w(w) {
    ...
    private:
        ...
        struct Node {
            Node(const State& state, Cost fScore, Cost gScore, Cost focalHeuristic): state(state), fScore(fScore), gScore(gScore), focalHeuristic(focalHeuristic) {}
    ...
    // typedef typename openSet_t::handle_type fibHeapHandle_t;
    typedef typename boost::heap::d_ary_heap<fibHeapHandle_t, boost::heap::arity<2>, boost::heap::mutable_<true>, boost::heap::compare<compareFocalHeuristic> > focalSet_t;
